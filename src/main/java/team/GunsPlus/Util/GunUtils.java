...
public class GunUtils {
	...
	public static HashMap<LivingEntity, Double> getTargets(Location l, PropertyHolder g,
			boolean zoom) {
		HashMap<LivingEntity, Double> targets = new HashMap<LivingEntity, Double>();
		Location loc = l.clone();
		HashMap<LivingEntity, Double> e = null;
	...
	public static HashMap<LivingEntity, Double> getTargetEntities(
		...
		while (bitr.hasNext()) {
			...
			for (LivingEntity e : entities) {
				...
				if (l.toVector().distance(blockcenter.toVector()) > el
					...
					targets.put(e, (double) (((Number) g.getProperty("HEADSHOTDAMAGE")).doubleValue() + changedamage<0?0:((Number) g.getProperty("HEADSHOTDAMAGE")).doubleValue() + changedamage) * -1);
				} else {
					targets.put(e, (double) (((Number) g.getProperty("DAMAGE")).doubleValue()<0?0:((Number) g.getProperty("DAMAGE")).doubleValue() + changedamage));
				}
			}
		}
		return targets;
	...
	public static void zoomOut(GunsPlusPlayer gp) {
		...
		sp.getMainScreen().removeWidget(PlayerUtils.getPlayerBySpoutPlayer(sp).getZoomtexture());
	}
